...
void page_init(void) {
	...
	u_long tmp = PADDR(freemem) / PAGE_SIZE;
	for(u_long i = 0; i < tmp; i++) {
		struct Page *var = &pages[i];
		var->pp_ref = 1;
	}
	/* Step 4: Mark the other memory as free. */
	/* Exercise 2.3: Your code here. (4/4) */
	for(u_long i = tmp; i < npage; i++) {
		struct Page *elm = &pages[i];
		elm->pp_ref = 0;
		LIST_INSERT_HEAD(&page_free_list, elm, pp_link);
	}
}
...
int page_alloc(struct Page **new) {
	...
	memset((void *)page2kva(pp), 0, PAGE_SIZE * sizeof(char));
...
static int pgdir_walk(Pde *pgdir, u_long va, int create, Pte **ppte) {
	...
	Pte *p = (Pte *)KADDR(PTE_ADDR(*pgdir_entryp));
	*ppte = &p[PTX(va)];
	return 0;
}
 ...
 *   page table entry should be set to 'perm | PTE_C_CACHEA
 BLE | PTE_V'.
 *
 * Post-Condition:
 *   Return 0 on success
