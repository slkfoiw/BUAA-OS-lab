...
int main() {
	...
	if (isSon) {
		if ((who = fork()) == 0) {
			...
			debugf("in grand son ptr0: %d\n", ptr[0]);
			
			ipc_send(i, 0, ptr, PTE_D | PTE_LIBRARY);
			
			debugf("in grandson ptr0: %d\n", ptr[0]);
			...
			debugf("before ptr: %x who:%d\n", ptr, who); 
			ipc_send(who, syscall_getenvid(), 0, 0);
			debugf("after ptr: %x who:%d\n", ptr, who); 
			...
			debugf("ptr: %x who:%d\n", ptr, who); 
			debugf("in son ptr:%d %d %d %d\n", ptr[0], ptr[100], ptr[200], ptr[300]);
